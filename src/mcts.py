...
class Node:
    def __init__(self, s, model, C, pi_dim=4, verbose=False, using_prior_for_exploration=False):
        ...
        if self.verbose:
            print('NEW NODE:', self.s.shape, self.NODE_ID)
        ...
        # self.Q = torch.zeros(self.pi_dim)  # The mean value of the next state (Q = W/N)
        # self.G = torch.zeros(self.pi_dim)  # Direct expected free energy, calculated by 'expand()'
        ...
        self.C = C  # A constant in AlphaGo Zero...
        self.in_progress = -1  # This takes the index of the child node that is currently under investigation.
    ...
    def probs_for_selection(self):
        ...
        if self.using_prior_for_exploration:
            return Qnormed + self.C * self.Qpi / (self.N + 1e-6)  # Avoid division by zero
        else:
            return Qnormed + self.C / (self.N + 1e-6)  # Avoid division by zero

    def select(self, deterministic=True):
        path = []
        actions_path = []
        if deterministic:
            self.in_progress = torch.argmax(self.probs_for_selection()).item()
        else:
            self.in_progress = torch.multinomial(self.probs_for_selection(), 1).item()
        actions_path.append(self.in_progress)
        path.append(self.children_nodes[self.in_progress])
        while all(child is not None for child in path[-1].children_nodes):  # If not leaf!
            if deterministic:
                path[-1].in_progress = torch.argmax(path[-1].probs_for_selection()).item()
            else:
                path[-1].in_progress = torch.multinomial(path[-1].probs_for_selection(), 1).item()
    ...
    def expand(self, use_means=False, samples=1):
        ...
        if self.pi_dim == 4:
            ...
            raise ValueError(f'Unsupported pi_dim: {self.pi_dim}')
        ...
        for i in range(self.pi_dim):
            self.children_nodes[i] = Node(s=ps_next[i], model=self.model, C=self.C, pi_dim=self.pi_dim,
                                          using_prior_for_exploration=self.using_prior_for_exploration)
        if self.verbose:
            print('Expand:', G.detach(), self.W, self.N)

    def backpropagate(self, path, G):
        if self.verbose:
            print('Back-propagate:', [p.NODE_ID for p in path], G.detach())
    ...
    def action_selection(self, deterministic=True):
        path = []
        if deterministic:
            path.append(torch.argmax(self.N).item())
        else:
            path.append(torch.multinomial(self.normalization(self.N), 1).item())
        node = self.children_nodes[path[-1]]
        if self.verbose:
            print(len(path), node.NODE_ID)
        while all(child is not None for child in node.children_nodes):
            if deterministic:
                path.append(torch.argmax(node.N).item())
            else:
                path.append(torch.multinomial(self.normalization(node.N), 1).item())
        ...
        while i < len(path) - 1:
            if self.pi_dim == 4:
                if (path[i] == 0 and path[i + 1] == 1) or (path[i] == 1 and path[i + 1] == 0) or \
                        (path[i] == 2 and path[i + 1] == 3) or (path[i] == 3 and path[i + 1] == 2):
                    i += 2
                else:
                ...
                raise ValueError(f'Unknown number of pi_dim {self.pi_dim}')
        if self.verbose:
            print('Action selection:', path, 'trimmed path:', trimmed_path)
        return trimmed_path

def calc_threshold(P, axis):
    return torch.max(P, axis=axis)[0] - torch.mean(P, axis=axis)

def normalization(x, tau=1):
    '''Compute softmax values for each set of scores in x.'''
    return torch.softmax(x / tau, dim=0)
...
def active_inference_mcts(model, frame, params, o_shape=(64, 64, 1)):
    ...
    qs0_mean, qs0_logvar = model.model_down.encoder(torch.tensor(frame.reshape(1, o_shape[0], o_shape[1], o_shape[2]), dtype=torch.float32))

    # Important to be the mean here as we repeat it model.pi_dim times!
    root = Node(s=qs0_mean[0].detach(), model=model, C=params.C, pi_dim=model.pi_dim, using_prior_for_exploration=params.using_prior_for_exploration)

    # Habit.
    root.Qpi = model.model_top.encode_s(qs0_mean)[1].detach().numpy()[0]
    ...
    for repeat in range(params.repeats):
        if calc_threshold(normalization(root.N), axis=0) > params.threshold:
            ...
            if params.verbose:
                print('Decision in phase B', np.round(root.probs_for_selection().numpy(), 2), np.round(MCTS_choices.numpy(), 2),
    ...
    final_path = root.action_selection(deterministic=True)
    if params.verbose:
        print('Decision in phase C', root.N.numpy(), len(all_paths), np.round(root.Q().numpy() - root.Q().min().item(), 2),
              np.round(root.C * root.Qpi.numpy() * np.sqrt(root.N.sum().item()) / (1.0 + root.N.sum().item()), 2), 'Qpi:', np.round(root.Qpi.numpy(), 2))
