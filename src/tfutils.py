...
def kl_div_loss_analytically_from_logvar_and_precision(mu1, logvar1, mu2, logvar2, omega):
    return 0.5 * (logvar2 - torch.log(omega) - logvar1) + (torch.exp(logvar1) + (mu1 - mu2).pow(2)) / (2.0 * torch.exp(logvar2) / omega) - 0.5

def kl_div_loss_analytically_from_logvar(mu1, logvar1, mu2, logvar2):
    return 0.5 * (logvar2 - logvar1) + (torch.exp(logvar1) + (mu1 - mu2).pow(2)) / (2.0 * torch.exp(logvar2)) - 0.5
...
def entropy_bernoulli(p, displacement=1e-5):
    return -(1 - p) * torch.log(displacement + 1 - p) - p * torch.log(displacement + p)

def log_bernoulli(x, p, displacement=1e-5):
