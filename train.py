...
# Import custom libraries (Assumed they are translated into PyTorch or compatible)
...
if optimizers == {}:
    optimizers['top'] = optim.Adam(model.model_top.parameters(), lr=l_rate_top)
    optimizers['mid'] = optim.Adam(model.model_mid.parameters(), lr=l_rate_mid)
    optimizers['down'] = optim.Adam(model.model_down.parameters(), lr=l_rate_down)
...
for epoch in range(start_epoch, epochs + 1):
    ...
    for i in range(ROUNDS):
        ...
        ps1_mean, ps1_logvar = loss.train_model_mid(
            model_mid=model.model_mid, s0=qs0, qs1_mean=qs1_mean, qs1_logvar=qs1_logvar, Ppi_sampled=pi0, omega=current_omega, optimizer=optimizers['mid']
        )

        # -- TRAIN DOWN LAYER --------------------------------------------------
        loss.train_model_down(
            model_down=model.model_down, o1=o1, ps1_mean=ps1_mean, ps1_logvar=ps1_logvar, omega=current_omega, optimizer=optimizers['down']
        )
    ...
    F_mid, loss_terms_mid, ps1, ps1_mean, ps1_logvar = loss.compute_loss_mid(
        model_mid=model.model_mid, s0=s0, Ppi_sampled=pi0, qs1_mean=qs1_mean, qs1_logvar=qs1_logvar, omega=var_a/2.0 + var_d
    )
    F_down, loss_terms, po1, qs1 = loss.compute_loss_down(
        model_down=model.model_down, o1=o1, ps1_mean=ps1_mean, ps1_logvar=ps1_logvar, omega=var_a/2.0 + var_d
    )
    ...
    stats['TC'].append(np.mean(total_correlation(qs1.detach().numpy())))
    ...
    reconstructions_plot(o0, o1, po1.detach().numpy(), filename=folder + '/imagination_' + signature + '_' + str(epoch) + '.png', colour=False)
    ...
    reconstructions_plot(o0, o1, po1.detach().numpy(), filename=folder + '/reward_imagination_' + signature + '_' + str(epoch) + '.png')
    mse_reward = u.compare_reward(o1=o1, po1=po1.detach().numpy())
    stats['mse_r'].append(mse_reward)
    stats_plot(stats, folder + '/1_result_' + signature)

    print('{0}, F: {1:.2f}, MSEo: {2:.3f}, KLs: {3:.2f}, omega: {4:.2f}+-{5:.2f}, KLpi: {6:.2f}, TC: {7:.2f}, dur. {8}s'.format(
        epoch, stats['F'][-1], stats['mse_o'][-1], stats['kl_div_s'][-1], stats['omega'][-1],
        stats['omega_std'][-1], stats['kl_div_pi'][-1], stats['TC'][-1], round(time.time() - start_time, 2)))
    start_time = time.time()
